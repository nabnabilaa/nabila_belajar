...
const DESC = {
  ...
  programming: ["Pseudocode tracing · IF/ELSE · SWITCH · loops · arrays · functions · recursion · Swift",
...
function TagChip({ q }) {
  if (!q || !q.tag) return null;
  const c = { Swift: ["#FF9F0A", "rgba(255,159,10,.15)"], Pseudocode: ["#0A84FF", "rgba(10,132,255,.15)"], Concept: ["#BF5AF2", "rgba(191,90,242,.15)"] }[q.tag] || ["#8E8E93", "rgba(255,255,255,.08)"];
  const lvl = q.level || 1;
  const dots = "●".repeat(lvl) + "○".repeat(3 - lvl);
  const lbl = { 1: "Easy", 2: "Medium", 3: "Harder" }[lvl];
  return <span style={{ display: "inline-block", fontSize: 11, fontWeight: 700, borderRadius: 99, padding: "3px 10px", color: c[0], background: c[1] }}>{q.tag} · <span style={{ letterSpacing: 1 }}>{dots}</span> {lbl}</span>;
}
...
export default function AcademyQuiz() {
  ...
  if (screen === "learn") return <Learn onHome={() => setScreen("home")} />;
  return <Home topic={topic} setTopic={setTopic} player={player} setPlayer={setPlayer} bestOf={bestOf} Badge={Badge} onStart={start} onLearn={() => setScreen("learn")} />;
}

function Home({ topic, setTopic, player, setPlayer, bestOf, Badge, onStart, onLearn }) {
  ...
  return (
    <div style={{ ...PAGE, maxWidth: 500, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap: "wrap" }}>
        {TOPIC_KEYS.map((k) => (
          <button key={k} style={{ ...tab(topic === k, COL[k] || "#5E5CE6"), minWidth: 150, lineHeight: 1.3 }} onClick={() => setTopic(k)}>
            {ICON[k]} {BANK.meta.topics[k].label}
            <span style={{ display: "block", fontWeight: 500, fontSize: 11, opacity: .8 }}>{BANK.meta.topics[k].label_id} · {BANK.meta.topics[k].total} soal</span>
          </button>
        ))}
      </div>
      ...
      <button onClick={onLearn} style={{ display: "block", width: "100%", margin: "14px auto 0", padding: 13, borderRadius: 13, background: "rgba(48,227,202,.12)", border: "1px solid rgba(48,227,202,.3)", color: "#7af0e0", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: FONT }}>📖 Study Material / Materi Belajar — Programming · OOP · Swift</button>
    </div>
  );
}

function Learn({ onHome }) {
  const [sec, setSec] = useState("programming");
  const [open, setOpen] = useState({});
  const SEC = { programming: "💻 Programming", oop: "🧩 OOP", swift: "🍎 Swift" };
  const cards = (BANK.learn[sec] || []);
  return (
    <div style={{ ...PAGE, maxWidth: 560, margin: "0 auto" }}>
      <div style={{ textAlign: "center", marginBottom: 8 }}>
        <div style={{ fontSize: 40 }}>📖</div>
        <h1 style={{ fontSize: 22, fontWeight: 800, margin: "4px 0 4px" }}>Study Material</h1>
        <p style={{ color: "#636366", fontSize: 13 }}>Materi belajar dasar — baca dulu sebelum latihan<br /><span style={{ color: IDCOL, fontStyle: "italic" }}>Read these foundations before practicing</span></p>
      </div>
      <div style={{ display: "flex", gap: 8, justifyContent: "center", flexWrap: "wrap", marginBottom: 14 }}>
        {["programming", "oop", "swift"].map((k) => (
          <button key={k} onClick={() => setSec(k)} style={{ padding: "9px 16px", borderRadius: 11, border: `1px solid ${sec === k ? "rgba(48,227,202,.5)" : "rgba(255,255,255,.09)"}`, background: sec === k ? "rgba(48,227,202,.18)" : "rgba(255,255,255,.05)", color: sec === k ? "#fff" : "#8E8E93", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: FONT }}>{SEC[k]}</button>
        ))}
      </div>
      {cards.map((c, i) => {
        const isOpen = open[sec + i] ?? (i === 0);
        return (
          <div key={i} style={{ background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.08)", borderRadius: 14, marginBottom: 10, overflow: "hidden" }}>
            <button onClick={() => setOpen((o) => ({ ...o, [sec + i]: !isOpen }))} style={{ width: "100%", textAlign: "left", background: "none", border: "none", padding: "14px 16px", color: "#F5F5F7", fontSize: 14.5, fontWeight: 700, cursor: "pointer", fontFamily: FONT, display: "flex", justifyContent: "space-between", gap: 10 }}>
              <span>{c.title}<small style={{ display: "block", fontWeight: 500, fontSize: 12, color: IDCOL, fontStyle: "italic", marginTop: 2 }}>{c.title_id}</small></span>
              <span style={{ color: "#636366" }}>{isOpen ? "▴" : "▾"}</span>
            </button>
            {isOpen && (
              <div style={{ padding: "0 16px 15px", fontSize: 13.5, lineHeight: 1.65, color: "#C7C7CC" }}>
                {c.body}
                {c.code && <Txt s={c.code} />}
                <span style={{ color: IDCOL, fontStyle: "italic", display: "block", marginTop: 7, paddingTop: 7, borderTop: "1px dashed rgba(255,255,255,.1)" }}>{c.body_id}</span>
              </div>
            )}
          </div>
        );
      })}
      <button onClick={onHome} style={{ width: "100%", marginTop: 14, padding: 13, borderRadius: 13, background: "rgba(255,255,255,.07)", border: "none", color: "#F5F5F7", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: FONT }}>← Back / Kembali</button>
    </div>
  );
}
  ...
  return (
    <div style={{ ...PAGE, maxWidth: 620, margin: "0 auto", display: "flex", flexDirection: "column", minHeight: "100vh" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <span style={{ color: "#636366", fontSize: 13 }}>{topicLabel(topic)} · S{session + 1}</span>
      ...
      <div style={{ background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.075)", borderRadius: 16, padding: "16px 18px", marginBottom: 13 }}>
        <div style={{ marginBottom: 10 }}><TagChip q={q} /></div>
  ...
  return (
    <div style={{ ...PAGE, maxWidth: 620, margin: "0 auto" }}>
      <div style={{ background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.075)", borderRadius: 20, padding: "26px 22px", textAlign: "center", marginBottom: 14 }}>
        <p style={{ color: "#636366", fontSize: 13, margin: "0 0 18px", display: "flex", gap: 8, justifyContent: "center", alignItems: "center" }}>{topicLabel(topic)} · Session {session + 1} · <Badge /></p>
